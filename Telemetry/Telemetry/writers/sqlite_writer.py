#!/usr/bin/env python3
"""SQLite writer for Project ARCHON Telemetry.

Implements the same channel contract as CsvWriter and stores every original
payload losslessly in payload_json while promoting frequently queried fields
...
class SQLiteWriter:
    ...
    def flush(self) -> None:
        """Persist the in-memory batch in one short transaction.

        Rows are buffered without touching SQLite during normal Observer ticks.
        The database write lock is acquired only while the batch is committed,
        so multiple Observer processes can take turns quickly.
        """
        with self._lock:
            self._ensure_open()
            if not self._buffer:
    ...
    def _execute_channel(
        ...
        *,
        ...
        with self._lock:
            ...
            self._buffer.append(
                (channel, sql, values, duplicate_policy)
            )
            self._pending = len(self._buffer)
