#!/usr/bin/env python3
"""Build deterministic STUDIO20.2 full and delta archives from STUDIO20.1."""
...
LIST = Path("Release/STUDIO20.2/packaging_whitelist.txt")
...
def main() -> int:
    ...
    old_list = baseline / "Release/STUDIO20.1/packaging_whitelist.txt"
    old_paths = set(rows(old_list)) if old_list.is_file() else set()
    delta = []
    for rel in paths:
        old = baseline / rel
        if rel not in old_paths or not old.is_file() or sha(root / rel) != sha(old):
            delta.append(rel)

    output.mkdir(parents=True, exist_ok=False)
    full = output / "ARCHON_STUDIO20.2_RELEASE.zip"
    patch = output / "ARCHON_STUDIO20.2_SCIENCEFIX2_DELTA.zip"
    archive(root, paths, full)
    archive(root, delta, patch)
    receipt = {
        "schema": "archon_studio20_2_artifacts_v1",
