#!/usr/bin/env python3
"""Build deterministic STUDIO20.3 full and source-policy delta archives from STUDIO20.2."""
from __future__ import annotations
import argparse, hashlib, json
from pathlib import Path
import zipfile
ROOT=Path(__file__).resolve().parents[1]
LIST=Path('Release/STUDIO20.3/packaging_whitelist.txt')
...
def main():
    ...
    old_list=baseline/'Release/STUDIO20.2/packaging_whitelist.txt'; old_paths=set(rows(old_list)) if old_list.is_file() else set()
    delta=[]
    for rel in paths:
        old=baseline/rel
        if rel not in old_paths or not old.is_file() or sha(root/rel)!=sha(old): delta.append(rel)
    output.mkdir(parents=True,exist_ok=False)
    full=output/'ARCHON_STUDIO20.3_RELEASE.zip'; patch=output/'ARCHON_STUDIO20.3_SOURCE_POLICY_DELTA.zip'
    archive(root,paths,full); archive(root,delta,patch)
    receipt={'schema':'archon_studio20_3_artifacts_v1','release_files':len(paths),'delta_files':len(delta),'artifacts':{full.name:sha(full),patch.name:sha(patch)}}
