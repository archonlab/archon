"""Read the canonical observational scientific view."""
...
def rule_set(records: Iterable[dict[str, Any]]) -> set[str]:
    return {
        ...
        and normalize_rule(record.get("rule")) != "unknown"
    }
...
def scientific_atlas_records(atlas: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if explicitly_routed:
        return [record for record in records if isinstance(record, dict) and record.get("is_scientific_observational") is True]
    return [record for record in records if isinstance(record, dict) and record.get("is_representative", True) and is_observational_eligible(record)]
