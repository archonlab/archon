#!/usr/bin/env python3
"""Compatibility CLI for modular morphological evolution analysis."""
...
from Analyzer_next.engines.evolution_engine import EvolutionEngine


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build morphology life-cycle timelines from Observer samples."
    )
...
def print_summary(
    paths: EvolutionPaths, result: EvolutionRunResult
) -> None:
    report = result.report
    print("Morphological Evolution Engine v1.0")
    print("=" * 64)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not results_dir.exists():
        ...
        return 1
    paths = EvolutionPaths(results_dir=results_dir)
    result = EvolutionEngine().run(paths)
