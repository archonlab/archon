#!/usr/bin/env python3
"""Compatibility CLI for modular morphological event detection."""
...
from Analyzer_next.core.events.contracts import EventPaths, EventRunResult
from Analyzer_next.engines.event_engine import EventEngine


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Detect decisive morphology events from Observer samples."
    )
    parser.add_argument("results_folder", help="Universe Search results folder.")
    return parser.parse_args(argv)


def print_summary(paths: EventPaths, result: EventRunResult) -> None:
    report = result.report
    print("Morphological Event Detector v1.0")
    print("=" * 64)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not results_dir.exists():
        ...
        return 1
    paths = EventPaths(results_dir=results_dir)
    result = EventEngine().run(paths)
