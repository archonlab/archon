"""Normalize cached telemetry rows and derive local morphology complexity."""
...
from Analyzer_next.core.evolution.constants import NUMERIC_COLUMNS
from Analyzer_next.core.evolution.numeric import clamp01, safe_float, safe_int
...
def local_mci(row: dict[str, Any]) -> float:
    ...
    return clamp01(
        ...
        * min(
            1.0,
            abs(safe_float(row.get("_morphology_aspect"), 1.0) - 1.0) / 5.0,
        )
    )
...
def normalize_rows(raw_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in raw_rows:
        ...
        rows.append(row)
    rows.sort(key=lambda row: row.get("_tick", 0))
    return rows
