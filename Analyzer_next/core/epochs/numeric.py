"""Small deterministic numeric helpers for epoch detection."""
...
def stdev(values: Iterable[float]) -> float:
    ...
    if len(items) < 2:
        return 0.0
    average = mean(items)
    return math.sqrt(sum((value - average) ** 2 for value in items) / (len(items) - 1))
...
def slope(values: list[float]) -> float:
    ...
    if count < 2:
        return 0.0
    middle_x = (count - 1) / 2.0
    middle_y = mean(values)
    denominator = sum((index - middle_x) ** 2 for index in range(count))
    if denominator <= 1e-12:
        return 0.0
    raw = sum(
        (index - middle_x) * (values[index] - middle_y)
...
def sparkline(values: list[float], width: int = 36) -> str:
    ...
    for value in items:
        ...
        output.append(chars[min(len(chars) - 1, int(round(normalized * (len(chars) - 1))))])
